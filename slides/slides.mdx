...
import ReactConcurrent from './images/egghead/ReactConcurrent.png'
import ReactSuspense from './images/egghead/ReactSuspense.png'
...
> 👉 React is the leading solution

> 👉 Vue is growing but not faster than React

> 👉 Angular is still a valid solution
...
> 👉 Only internal changes, no breaking changes for you.
...
> 👉 Very valuable for SEO and perceived performance
...
> 👉 React first collects the changes in the render step, then commit the changes in a batch.
...
> 👉 You can rewrite your code step by step 🤗
...
<dl>
  <dt>React Hook</dt>
  <dd>
    <em>/ɹiːˈækt/ /hʊk/</em> • A special function that allows to interact with
...
> 👉 It's better to use a function when you need to consider the previous value.
...
> 👉 You can create a non-controlled component with a controlled component but not the other way around.

---

## To take away

> 👉 Controlled components are prefered 👮‍♂️
...
<dl>
  <dt>Effect</dt>
  <dd>
    <em>\e.fɛ\</em> • A side effect function called after the commit phase.
  </dd>
</dl>
...
> 👉 The dependencies are the external values used in the effect.
...
> 👉 No dependencies, it's only run during the Mount phase!
...
> 👉 Use error boundaries at strategical places.
...
> 👉 The render props still have their use cases (as the HOCs)

> 👉 The render props allow the isolation of the rendering

> 👉 The render props allow a fine grain handling of the errors
...
> 👉 Decouple the action from the output

> 👉 You can test the reducer logic
...
# “The card title should be on a green background, it's better”
...
# “Could display a small ad banner instead of the card title?”
...
- Is the injected value different?
  - Yes → The `Consumer` are rerender, deep in the React tree
  - No → Nothing happens
...
## How to access ref inside an element <br /> while exposing it?
...
- Problem n°1: [repetitive calls](https://codesandbox.io/s/serene-wescoff-cxg1w)
- Problem n°2: [no update notification](https://codesandbox.io/s/keen-kirch-qkrhs)
...
[Demo](https://codesandbox.io/s/optimistic-brook-4123e)

---

## useLayoutEffect

- The signature is identical to useEffect, <br />but it fires **synchronously** after all DOM mutations.
- [Different use cases](https://github.com/mui-org/material-ui/search?utf8=%E2%9C%93&q=useLayoutEffect&type=)

---

## useDebugValue

- [Only for custom Hooks](https://codesandbox.io/s/clever-chatterjee-mec9q)
- I have never used it 🤷‍♂️.

---

<Title>
  <Title.Picture src={ReactConcurrent} />
  <Title.Text>Concurrent Mode</Title.Text>
</Title>
...
<iframe
  style={{ width: '80vw', height: '50vh', overflow: 'hidden' }}
  src="https://www.youtube.com/embed/ByBPyMBTzM0"
  frameborder="0"
  allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
  allowfullscreen
/>

Last presented by Andrew Clark and Brian Vaughn during the React Conf 2018.
...
No breaking changes, so it should conceptually be as simple as something like this:
...
⚠️ The API is very likely to change.

---

## Require Strict Mode

Wrap your component subtrees in `<React.StrictMode>`
...
- [String ref](https://reactjs.org/docs/refs-and-the-dom.html#legacy-api-string-refs) -> useRef
...
function Slider(props) {
  ...
  valueRef.current = value // Wrong ❌

  const handleKeyDown = React.useCallback(
    event => {
      const value = valuesRef.current;

      // 👍 no need for `keyCode?: number`
      switch (event.key) {
        case 'Home':
      // …
    }, []);
...
- Prerender content in the background
- An [existing](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/hidden) HTML attribute (close to aria-hidden)
- First presented in [this issue](https://github.com/oliviertassinari/react-swipeable-views/issues/453#issuecomment-417939459):
...
[Live demo](https://codesandbox.io/s/material-demo-w5hwx)

---

## How to access a DOM node in a hook?

Let's say I want to output the height of an element.
...
<Title>
  <Title.Picture src={ReactSuspense} />
  <Title.Text>Suspense for Data Fetching</Title.Text>
</Title>
...
[Live demo](https://codesandbox.io/s/material-demo-297gn)
